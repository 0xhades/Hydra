            ...
            try
            {
                ...
                if (_tcpClientCached == null)
                {
                    ...
                    if (_TimeoutInSeconds > 0) {
                        ...
                        var result = _tcpClient.BeginConnect(_proxyHost, _proxyPort, null, null);
                        var success = result.AsyncWaitHandle.WaitOne(TimeSpan.FromSeconds(_TimeoutInSeconds));
                        if (!success)
                        {
                            throw new ProxyException("timeout");
                        }
